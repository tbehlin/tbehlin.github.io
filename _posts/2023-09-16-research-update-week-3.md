...
    Summing this by year, we can see a plot that resembles the plots given in the AFPI report. One comment on this plot is that it does obscure many of the details of each year, and the resulting plots I created do mirror the general trend that their plot shows, although the exact numbers don't match, which is what we expected.
